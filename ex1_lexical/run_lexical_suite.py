#!/usr/bin/env python3
"""Run the Exercise 1 lexical fixture suite against an Exercise 5 compiler."""
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--root", default=Path("tests/ex1_lexical"), type=Path)
    parser.add_argument("--case")
    parser.add_argument(
        "--category",
        choices=("self-check", "additional", "official"),
...
def discover_cases(root: Path, timeout_seconds: float) -> list[TestCase]:
    ...
    self_check_inputs = root / "self-check-ex1" / "tests"
    self_check_expected = root / "self-check-ex1" / "expected_output"
    for input_path in sorted(self_check_inputs.glob("*.txt")):
        cases.append(
            TestCase(
                id=f"self_check_{input_path.stem}",
                category="self-check",
                input_path=input_path,
                expected_path=preferred_expected_path(
                    self_check_expected / input_path.with_suffix(".json").name,
                    self_check_expected / input_path.name,
                ),
                timeout_seconds=timeout_seconds,
            )
        )

    additional_inputs = root / "additional-self-check-ex1" / "input"
    additional_expected = root / "additional-self-check-ex1" / "expected_output"
    for input_path in sorted(additional_inputs.glob("*.txt")):
        cases.append(
            TestCase(
                id=f"additional_{input_path.stem}",
                category="additional",
                input_path=input_path,
                expected_path=preferred_expected_path(
                    additional_expected / f"{input_path.stem}_Expected_Output.json",
                    additional_expected / f"{input_path.stem}_Expected_Output.txt",
                ),
                timeout_seconds=timeout_seconds,
            )
        )

    official_root = root / "official-tests"
    for input_path in sorted(official_root.glob("*/*.txt")):
        if input_path.name == "expected_output.txt":
            continue
        cases.append(
            TestCase(
                id=f"official_{input_path.parent.name}",
                category="official",
                input_path=input_path,
                expected_path=preferred_expected_path(
                    input_path.parent / "expected_output.json",
                    input_path.parent / "expected_output.txt",
                ),
                timeout_seconds=timeout_seconds,
            )
        )
...
def preferred_expected_path(structured_path: Path, raw_path: Path) -> Path:
