#!/usr/bin/env python3
"""Run the Exercise 3 semantic fixture suite against an Exercise 5 compiler."""
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--root", default=Path("tests/ex3_semantic"), type=Path)
    parser.add_argument("--case")
    parser.add_argument(
        "--category",
        help=(
            "Run one source corpus, e.g. official, official-additional, "
            "unofficial/03_arrays, or unofficial/10_complex."
        ),
    )
    parser.add_argument(
        "--mode",
        choices=("compiler", "runtime"),
        help="Run only semantic-error compiler checks or valid-program runtime checks.",
    )
...
def discover_cases(root: Path, timeout_seconds: float) -> list[TestCase]:
    ...
    for suite in ("official", "official-additional"):
        input_dir = root / suite / "input"
        expected_dir = root / suite / "expected_output"
        expected_paths = sorted(
            [*expected_dir.glob("*_Expected_Output.txt")]
            + [*expected_dir.glob("*_Expected_Output.json")]
        )
        for expected_path in expected_paths:
            ...
            cases.append(
                make_case(
                    category=category,
                    case_id=case_id_from_parts((category, stem)),
    ...
    unofficial_expected = root / "unofficial" / "expected_output"
    unofficial_input = root / "unofficial" / "input"
    expected_paths = sorted(
        [*unofficial_expected.glob("*_Expected_Output.txt")]
        + [*unofficial_expected.glob("*_Expected_Output.json")]
    )
    for expected_path in expected_paths:
        stem = expected_stem(expected_path)
        matches = sorted(unofficial_input.glob(f"*/{stem}.txt"))
        if len(matches) != 1:
            raise FileNotFoundError(
                f"expected exactly one input for unofficial expected output: "
                f"{expected_path}"
            )
        input_path = matches[0]
        category = f"unofficial/{input_path.parent.name}"
        cases.append(
            make_case(
                category=category,
                case_id=case_id_from_parts(("unofficial", input_path.parent.name, stem)),
...
def make_case(
    *,
    ...
    if legacy_text == "OK":
        ...
        raise ValueError(
            f"unsupported legacy expected output in {legacy_expected_path}: "
            f"{legacy_text!r}"
        )
